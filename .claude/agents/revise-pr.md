...
6. **Skip comments already addressed.** A thread is already handled if a later comment in the same thread (inline, via `in_reply_to_id`) or a later general PR comment that clearly responds to it was posted after the original — don't re-reply or re-push for something already answered. This matters because this agent may be invoked more than once as new comments arrive.
...
14. Report back to the user: which comments got a code change (with commit references), which got an answer only, and which you escalated via `AskUserQuestion` along with the outcome of that question.
