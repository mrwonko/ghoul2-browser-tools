...
3. Fetch the story: `gh issue view <number>` (accept either a bare number or a full issue URL from the invoker). If it has a parent epic, `gh issue view <epic-number>` too, for context on where this story fits.
4. Research what's needed to implement it:
   ...
   - `gh issue list --state all` / `gh pr list --state all --search <keywords>` for related prior art or decisions not yet written down in `decisions/`.
...
6. Write the plan as markdown, in the same spirit as the `decisions/` files: concrete decisions with the reasoning behind them, not just a task list. Include:
   ...
   - If this plan establishes a new non-obvious decision worth remembering, say so explicitly and suggest the filename it should get under `decisions/` — don't create the file yourself, that happens once the plan is accepted.
7. Post the plan as a comment on the issue: `gh issue comment <number> --body-file -` (pipe the markdown in). Then also return the same plan as your final response so the invoking conversation has it inline.
