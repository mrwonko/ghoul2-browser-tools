...
7. Post the plan as a comment on the issue. Write the plan to a file first (e.g. a heredoc redirect, `cat > /tmp/plan_body.md <<'EOF'` ... `EOF`), then pass that file straight to `--body-file`: `gh issue comment <number> --body-file /tmp/plan_body.md`. Never inline the plan into a double-quoted `--body "..."` argument, since plan text routinely contains `` ` ``, `$`, and other shell metacharacters (code snippets, shell examples) that would otherwise get interpolated. Also never do `gh issue comment <number> --body-file - <<'EOF'` with `$(cat /tmp/plan_body.md)` as the heredoc's only line — a *quoted* heredoc delimiter (`<<'EOF'`) disables all expansion, so `$(...)` is never evaluated and the literal string `$(cat /tmp/plan_body.md)` gets posted as the comment instead of the file's contents. If the plan is already in a file, `--body-file <path>` needs no heredoc/stdin indirection at all. Then also return the same plan as your final response so the invoking conversation has it inline.
