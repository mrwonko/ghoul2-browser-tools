...
3. Fetch the issue and its comments: `gh issue view <number> --json title,body,comments` (a bare number is safest — it resolves against the repo in the current directory; if handed a URL, confirm it points at *this* repo before trusting it). Prefer this over `gh api` for anything `gh issue view`/`gh pr view` already cover — a bare `gh api` call is hard to whitelist individually since it can hit almost any endpoint (including writes), whereas `gh issue view`/`gh pr view` are narrowly scoped to their target issue/PR. This JSON form also gives you what step 5 needs (`authorAssociation`, `id`, `reactionGroups` per comment) without a second fetch.
...
5. **Verify approval before writing any code.** `gh` authenticates as the repo owner's own account for everything it posts, so every comment/reaction shows the same `login` regardless of whether an agent or the human posted it (`authorAssociation` is always `OWNER`, `viewerDidAuthor` is always `true`) — comparing "author of the reaction" to "author of the plan" tells you nothing. What you *can* check is write access, which filters out a rando reacting/commenting on a public issue. The comments array from step 3 already has each comment's `authorAssociation` and `reactionGroups` (reaction counts by type, but no reactor logins) — no need to re-fetch comments via `gh api`. Only fall back to `gh api` for what step 3's JSON can't give you:
   ```sh
   gh api repos/{owner}/{repo}/issues/comments/<plan-comment-id>/reactions    # only if checking a reaction: gives reactor logins, which reactionGroups above omits
   gh api repos/{owner}/{repo}/collaborators/<login>/permission               # {"permission": "admin"|"write"|"read"|"none", ...} — no gh subcommand equivalent exists
   ```
   ...
   - A comment posted *after* the plan comment whose `authorAssociation` is not `NONE` — treat this as approval-with-amendments: fold the requested changes into your mental model of the plan before implementing (the amendment comment's text is now part of "the plan").
