...
6. **Keep every issue short.** Title + one short paragraph description + a dependency line if relevant — not a spec. A story issue should read in well under a minute; implementation detail belongs in `story-to-plan`'s output, not here. Present the finalized story list to the user as plain text first and get their go-ahead before filing anything on GitHub. If the user corrects the scope, revise and re-present rather than filing on a stale understanding.
