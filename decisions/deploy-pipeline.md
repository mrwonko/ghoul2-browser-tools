...
The `authorized_keys` entry for the CI-held private key restricts it (via `command="rrsync -wo <dir>"` plus `restrict`) to write-only rsync into one dedicated directory, rather than an unrestricted shell command or a read-write grant. `-wo` means a leaked key can overwrite the deployed site but can never read anything back over that connection; `rrsync`'s directory argument is enforced server-side regardless of what the client sends, so the directory boundary can't be widened from the CI side even if `deploy.sh`/secrets were compromised. This is why `deploy.md` calls out that the target directory must be used for nothing else — `rsync --delete` treats anything else placed there as drift to be removed.
