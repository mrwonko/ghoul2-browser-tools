...
`dist/index.html` is produced by Eleventy from `site/_includes/base.njk` (the shared `<html>`/`<head>` shell, common to every page using that layout) plus `site/index.njk` (that page's own front matter + body), with `postbuild` running `npx eleventy`. There's no checked-in HTML file that already has the right output-relative paths baked in to just copy verbatim; generating the page from a shared layout is the templating system's actual job, and it's what let a second page (`site/jk3-to-jk2/index.njk`) reuse `base.njk` (and anything added to it, like the site-wide footer and nav) without duplicating markup — see `decisions/multi-page-relative-links.md` for how links stay correct once pages live at different directory depths.
