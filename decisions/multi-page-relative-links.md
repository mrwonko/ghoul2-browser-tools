...
It lives under `site/_lib/` (underscore-prefixed, so Eleventy ignores it as a page source, the same as `_includes`/`_data`) rather than inline in `eleventy.config.js`, specifically so it has a plain colocated unit test (`site/_lib/fromRoot.test.js`) instead of only being covered indirectly through the visual-regression suite. `npm test`'s `vitest run src site` was widened from `src`-only to pick this up — `site/` still isn't part of `tsconfig.json`'s `rootDir` (see `decisions/eleventy-build.md`), this only concerns which directories vitest scans for `*.test.js`, not tsc/Eleventy's file discovery.
