...
- **`src/animationCfg.ts`** — `parseAnimationCfg(source: string)` generator layered on `commonTokenizer.ts`, grouping its token stream into `animation.cfg`'s `anim-entry` records (`reference/animation_cfg_grammar.abnf` lines 89-97): `AnimEntryKind.AnimEntry` (all 5 fields present) vs. `AnimEntryKind.IncompleteAnimEntry` (a trailing record cut short by EOF/embedded NUL, flagged with `AnimEntryWarning.IncompleteEntry`). Each entry also carries `tokens: readonly CommonToken[]` — every token consumed while collecting it, including skipped separators, so `entry.tokens.flatMap(t => t.warnings)` recovers every warning without a second `tokenize()` pass. `parseAnimInteger`/`parseAnimNumber` hand-parse `integer`/`number` fields per the grammar's `atoi`/`atof`-like semantics rather than delegating to `parseInt`/`parseFloat`. See `decisions/anim-entry-parser.md`.
...
- **`src/testdata/`** — `simpleskel.gla` (3-bone skeleton: root/foo/bar), `testmodel.glm` (2 surfaces, 2 LODs), `simple-animation.cfg` (a handful of plain anim-entry records, no comments/quotes), and `animation-duplicate-and-truncated.cfg` (duplicate `anim-name`s left unmerged, plus a truncated trailing record) used by unit tests.
...
- `decisions/anim-entry-parser.md` — `parseAnimationCfg` is syntax-only (no anim-name-table lookup, `fps == 0 → 1` default, or `frameLerp`); `IncompleteAnimEntry` as a distinct `Kind` instead of a bolted-on warning, and why it doesn't reproduce the original engine's silent `atoi("") == 0` fill-in; no synthetic terminal marker unlike `commonTokenizer`'s `Eof`; hand-rolled `integer`/`number` parsing instead of `parseInt`/`parseFloat`; fields store the raw `AnimFieldToken`, not a derived value; `tokens` carries every token (including skipped separators) an entry consumed, so no warning is lost; the empty-quoted-`anim-name` divergence from the original engine's accidental truncation.
